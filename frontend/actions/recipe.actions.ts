...
interface RecipeIngredient {
    item: string;
    amount: string;
    category: "Protein" | "Vegetable" | "Spice" | "Dairy" | "Grain" | "Other" | string;
}
...
interface RecipeActionResult {
    success: true;
    recipe: GeneratedRecipeData | StrapiRecipeEntity | Record<string, unknown>;
    recipeId: number | string;
    isSaved: boolean;
    fromDatabase: boolean;
    isPro: boolean;
    message: string;
    recommendationsLimit?: "unlimited" | number;
}

type RecipesByPantryIngredientsResult =
    | {
        success: true;
...
interface StrapiRecipeEntity {
    id: number | string;
    attributes?: Record<string, unknown>;
}

interface SavedRecipeEntity {
    id: number | string;
...
export async function getOrGenerateRecipe(formData: FormData): Promise<RecipeActionResult> {
    try {
        const user = (await checkUser()) as User | null;
        if (!user) {
        ...
        if (searchResponse.ok) {
            const searchData = (await searchResponse.json()) as StrapiListResponse<StrapiRecipeEntity>;
            ...
            if (searchData.data && searchData.data.length > 0) {
                ...
                if (savedRecipeResponse.ok) {
                    const savedData = (await savedRecipeResponse.json()) as StrapiListResponse<unknown>;
...
`;
        ...
        try {
            const cleanText = text
                ...
                .replace(/```\n?/g, "")
                .trim();
            recipeData = JSON.parse(cleanText) as GeneratedRecipeData;
        } catch (parseError) {
        ...
        const createdRecipe = (await createRecipeResponse.json()) as { data: { id: number | string } };
...
export async function saveRecipeToCollection(formData: FormData) {
    try {
        ...
        if (existingResponse.ok) {
            const existingData = (await existingResponse.json()) as StrapiListResponse<unknown>;
        ...
        const savedRecipe = (await saveResponse.json()) as { data: { id: number | string } };
        ...
        throw new Error(getErrorMessage(error) || "Failed to save recipe");
    }
}
...
export async function removeRecipeFromCollection(formData: FormData) {
    try {
        ...
        const searchData = (await searchResponse.json()) as StrapiListResponse<SavedRecipeEntity>;
        ...
        throw new Error(getErrorMessage(error) || "Failed to remove recipe");
    }
}

// Get recipes based on pantry ingredients
export async function getRecipesByPantryIngredients(): Promise<RecipesByPantryIngredientsResult> {
    try {
        const user = (await checkUser()) as User | null;
        if (!user) {
            throw new Error("User not authenticated");
        ...
        const pantryData = (await pantryResponse.json()) as StrapiListResponse<{ name: string }>;
...
`;
        ...
        try {
            const cleanText = text
                ...
                .replace(/```\n?/g, "")
                .trim();
            recipeSuggestions = JSON.parse(cleanText) as RecipeSuggestion[];
        } catch (parseError) {
            console.error("Failed to parse Gemini response:", text);
            throw new Error(
        ...
        throw new Error(getErrorMessage(error) || "Failed to get recipe suggestions");
    }
}
...
export async function getSavedRecipes(): Promise<{
    ...
    try {
        ...
        const data = (await response.json()) as { data: Array<{ recipe?: unknown }> };

        // Extract recipes from saved-recipes relations
        const recipes = data.data
            .map((savedRecipe) => savedRecipe.recipe)
            .filter(Boolean) as unknown[]; // Remove any null recipes
        ...
        throw new Error(getErrorMessage(error) || "Failed to load saved recipes");
    }
}